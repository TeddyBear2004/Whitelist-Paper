...
public record PunishCommand(String normalPermission, String extendedPermission, String type,
                            String verb, SessionFactory sessionFactory,
                            LuckPerms api) implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (sender.hasPermission(extendedPermission)) {
            ...
            if (sender instanceof Player player) {
                if (api != null) {
                    User target = api.getUserManager().getUser(uuid);
                    User source = api.getUserManager().getUser(player.getUniqueId());

                    if (source != null && target != null) {
                        Group targetGroup = api.getGroupManager().getGroup(target.getPrimaryGroup());
                        Group sourceGroup = api.getGroupManager().getGroup(source.getPrimaryGroup());

                        if (targetGroup != null && sourceGroup != null) {
                            if (targetGroup.getWeight().isPresent() && sourceGroup.getWeight().isPresent()) {
                                int targetInt = targetGroup.getWeight().getAsInt();
                                int sourceInt = sourceGroup.getWeight().getAsInt();

                                if (targetInt > sourceInt) {
                                    sender.sendMessage("Du kannst diesen Spieler nicht bestrafen!");
                                    return true;
                                }
                            }
                        }
    ...
    private void sendSuccessMessage(CommandSender sender, String player, long time, String reason) {
        ...
        Component advancedMessage = Component.text()
                .append(Component.text(player.trim(), NamedTextColor.RED))
